...
namespace TerraZ.Client
{
	public static class ClientUtils
	{
		...
		{
			Type[] p = TZLauncher.Launcher.Terraria.GetType("Main").GetValue<Type[]>("player");
			Rectangle rectangle = new Rectangle(
				(int)((float)TZLauncher.Launcher.Terraria.GetType("Main").GetValue<int>("mouseX") + TZLauncher.Launcher.Terraria.GetType("Main").GetValue<Vector2>("screenPosition").X), 
				(int)((float)TZLauncher.Launcher.Terraria.GetType("Main").GetValue<int>("mouseY") + TZLauncher.Launcher.Terraria.GetType("Main").GetValue<Vector2>("screenPosition").Y), 1, 1);
			for (int i = 0; i < 255; i++)
			{
				if (p[i].GetValue<bool>("active") && TZLauncher.Launcher.Terraria.GetType("Main").GetValue<int>("myPlayer") != i && !p[i].GetValue<bool>("dead"))
				{
